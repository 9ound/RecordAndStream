...
const videoFeedOne      = document.querySelector('video#onlineVideoA'),
      ...
      showStreamOne      = document.getElementById('showStreamOne'),
      showStreamTwo      = document.getElementById('showStreamTwo'),
...
let isSpeakingStateA = Bool(false),
    isSpeakingStateB = Bool(false),
    initSpeechDetect,

    f1LowMicA, f1HighMicA, f2LowMicA, f2HighMicA, f3LowMicA, f3HighMicA,
    f1LowMicB, f1HighMicB, f2LowMicB, f2HighMicB, f3LowMicB, f3HighMicB,
...
window.onload = function() {
  ...
  initButtonA.addEventListener('click', function() {
    ...
    // Updates canvas at 30fps
    // startDraw = setInterval(drawToCnvs, 33.33333333);
    });
  })
  ...
  isSpeakingStateA.addListener(function (e) {
    if (e.oldValue != e.newValue) {
      ...
      // console.log('isSpeakingStateA: ' + e.newValue);
    }
  });
  isSpeakingStateB.addListener(function (e) {
    if (e.oldValue != e.newValue) {
      ...
      // console.log('isSpeakingStateB: ' + e.newValue);
    }
  });
...
// Contents will be updated in micLoop()
...
function evtLoopFeedOne() {
  ...
  if(stop == false){
    ...
    if (evtOneElapsed > evtOneInterval) {
      ...
      // initial value stating that utterance is not detected
      let detectUtterance = false;

      // freqAmplitudes as float or byte
      // analyserA.getFloatFrequencyData(fDataA);
      ...
      let userFormants   = userCalibratedPeaks(f1BoxA, f2BoxA, f3BoxA, f1L, f1H, f2L, f2H, f3L, f3H, fByteDataA);
      ...
      if (signalStrength > micSensitivityA.value && similarBand && hasThreePeaks && userFormants){
...
function evtLoopFeedTwo() {
  ...
  if(stop == false){
    window.requestAnimationFrame(evtLoopFeedTwo);
    // calc elapsed time since last loop
    evtTwoNow = Date.now();
    ...
    if (evtTwoElapsed > evtTwoInterval) {
        ...
        let userFormants   = userCalibratedPeaks(f1BoxB, f2BoxB, f3BoxB, f1L, f1H, f2L, f2H, f3L, f3H, fByteDataB);
        ...
        // 2. ratio of 300 - 3400Hz is large
        // 3. peak freq bins similar to human voice
        if (signalStrength > micSensitivityB.value && similarBand && hasThreePeaks && userFormants){
          detectUtterance = true;
          console.log(detectUtterance)
        } else {
          detectUtterance = false;
        }
...
// arr.shift(), arr.push(detectUtterance);
